...
The system SHALL require `username` to be unique.

#### Scenario: Duplicate username
- **GIVEN** an existing user already has username `alice`
...
- **THEN** the system SHALL NOT create another user with the same username

### Requirement: Unique optional nickname
The system SHALL allow `nickname` to be omitted or blank, but provided nickname values MUST be unique and MUST NOT exceed 30 characters.

#### Scenario: Duplicate nickname
- **GIVEN** an existing user already has nickname `Ace`
- **WHEN** a client submits a registration request with nickname `Ace`
- **THEN** the system SHALL reject the request with `409 Conflict`
- **THEN** the system SHALL NOT create another user with the same nickname
