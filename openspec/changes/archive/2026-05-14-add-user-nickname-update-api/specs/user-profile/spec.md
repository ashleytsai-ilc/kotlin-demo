...
The system SHALL require a valid bearer access token to update the current user profile.
...
The system SHALL require non-empty updated nickname values to be unique across users.

#### Scenario: Duplicate nickname
- **GIVEN** another user already has nickname `Ace`
...
#### Scenario: Multiple users without nickname after update
- **GIVEN** another user has no nickname
