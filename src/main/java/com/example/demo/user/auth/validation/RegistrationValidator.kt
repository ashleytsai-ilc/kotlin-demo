...
class RegistrationValidator {
    fun validate(request: RegistrationRequest): ValidatedRegistration {
        ...
        if (username == null || username.isEmpty()) {
            ...
            if (!USERNAME_PATTERN.matcher(username).matches()
                || username.length < RegistrationValidationRules.USERNAME_MIN_LENGTH
                || username.length > UserAccountConstraints.USERNAME_MAX_LENGTH
            ) {
                errors.add(RegistrationValidationError.USERNAME_INVALID.toDetail())
            }
        }

        if (nickname != null) {
            if (nickname.isBlank()
                || RegistrationValidationRules.containsWhitespace(nickname)
                || nickname.length > UserAccountConstraints.NICKNAME_MAX_LENGTH
            ) {
                errors.add(RegistrationValidationError.NICKNAME_INVALID.toDetail())
            }
        }
    ...
    companion object {
        private val USERNAME_PATTERN: Pattern = Pattern.compile(RegistrationValidationRules.USERNAME_ALLOWED_PATTERN)
